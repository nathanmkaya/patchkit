 ...
 *
 * This interface abstracts database operations to support different SQLite drivers and
 * implementations while maintaining consistent behavior across platforms. It focuses on the
 * essential operations needed for patch execution.
 *
...
interface SqliteEngine {
     ...
     *
     * Used primarily for condition evaluation in preconditions and postconditions. Returns null if
     ...
     */
    suspend fun queryScalar(sql: String, args: List<SqlArg> = emptyList()): SqlScalar?

    /**
     * Execute a DML statement and return the number of affected rows.
     *
     * Used for INSERT, UPDATE, DELETE operations within patch actions. Returns the number of rows
     * modified, following SQLite's `changes()` behavior.
     *
     ...
     */
    suspend fun execute(sql: String, args: List<SqlArg> = emptyList()): Int
}

/**
 * Interface for managing database transaction boundaries.
 *
 * Provides ACID transaction management with support for both immediate and deferred transaction
 * modes. All implementations must ensure proper COMMIT/ROLLBACK handling.
 */
interface TransactionManager {
    /**
     * Execute a block of operations within a database transaction.
     *
     * The transaction automatically commits on successful completion and rolls back if any
     ...
     */
    suspend fun <T> inTransaction(immediate: Boolean = true, block: suspend () -> T): T
}

/**
 * Combined interface providing both SQL execution and transaction management.
 *
 * This is the primary interface required by PatchKit for complete database operations. It combines
 * basic SQL execution with transaction management capabilities.
 */
interface TransactionalEngine : SqliteEngine, TransactionManager

/**
 * Factory interface for lazily providing database engines.
 *
 * Allows PatchKit to resolve database connections on-demand rather than requiring pre-initialized
 * connections. This supports use cases where databases may not be immediately available or need
 * per-operation setup.
 */
