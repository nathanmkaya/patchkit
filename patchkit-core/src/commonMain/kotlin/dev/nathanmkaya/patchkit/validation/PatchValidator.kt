 ...
 *
 * Validators form a validation chain that checks patches for security, integrity, and policy
 * compliance before they are executed. Each validator can reject a patch by returning a
 * ValidationResult.Failure with an appropriate error code and message.
 *
 * Validators run in the order they are registered and the first failure stops the validation chain,
 * preventing patch execution.
 *
 * ## Built-in Validators:
...
fun interface PatchValidator {
     ...
     */
    suspend fun validate(patch: Patch, rawBytes: ByteArray?): ValidationResult
}

/**
 * Result of patch validation.
 *
 * Validators return either Success to allow execution to proceed, or Failure with a
 * machine-readable error code and human-readable message to prevent execution.
 */
sealed interface ValidationResult {
     ...
     */
    data class Failure(val code: String, val message: String) : ValidationResult
}
