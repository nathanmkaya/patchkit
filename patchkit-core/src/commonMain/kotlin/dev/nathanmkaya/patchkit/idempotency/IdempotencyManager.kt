 ...
 *
 * Idempotency management ensures that patches are applied exactly once, even if the same patch is
 * submitted multiple times. This is critical for maintaining data consistency and preventing
 * unintended side effects from repeated operations.
 *
...
interface IdempotencyManager {
     ...
     *
     * This method is called before checking or recording patch applications. It should create any
     * necessary tables, indexes, or other storage structures. Must be safe to call multiple times
     * (should not fail if structures already exist).
     *
     ...
     *
     * This check happens before patch execution to determine if the patch should be skipped. Only
     * patches that completed successfully should be considered "applied" - failed attempts should
     * not prevent retry.
     *
     ...
     */
    suspend fun hasBeenApplied(patchId: String, engine: SqliteEngine): Boolean

    /**
     * Record that a patch was successfully applied.
     *
     * This method is called only after successful patch execution and transaction commit. It marks
     * the patch as applied to prevent future duplicate execution.
     *
     ...
     */
    suspend fun recordApplication(patchId: String, engine: SqliteEngine, metadata: String? = null)
}
