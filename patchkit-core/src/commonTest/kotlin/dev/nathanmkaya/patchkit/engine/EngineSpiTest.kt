...
class EngineSpiTest {
    @Test
    fun transaction_commits_on_success_and_rolls_back_on_failure() = runTest {
        val eng = FakeEngine()
        ...
        assertFailsWith<IllegalStateException> {
            eng.inTransaction(immediate = false) { error("boom") }
        }
    ...
    @Test
    fun queryScalar_and_execute_record_calls_and_return_programmed_values() = runTest {
        val eng = FakeEngine()
    ...
    fun requireLong_fails_on_null() {
        assertFailsWith<IllegalStateException> { (null as SqlScalar?).requireLong("test") }
    }
}
