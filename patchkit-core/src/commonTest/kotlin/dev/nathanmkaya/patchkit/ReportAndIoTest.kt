...
class ReportAndIoTest {
    ...
    @Test
    fun apply_from_string_and_file_and_pretty_print() = runTest {
        // DB setup (bundled driver)
        val dbPath = (createTempDirectory() / "test.db")
        val conn = BundledSQLiteDriver().open(dbPath.toString())
        conn.execSQL(
            "CREATE TABLE users(id INTEGER PRIMARY KEY, name TEXT, active INTEGER DEFAULT 0)"
        )
        ...
        assertTrue(
            again.all { !it.success && it.events.any { e -> e.code == EventCode.IDEMPOTENT_SKIP } }
        )
    }

    private fun writeText(path: Path, contents: String) {
    ...
    private fun writeTextFile(
        ...
        fs.write(path, mustCreate = false) { writeUtf8(text) }
    }
    ...
    private fun randomHex(n: Int): String =
        buildString(n) { repeat(n) { append("0123456789abcdef"[Random.nextInt(16)]) } }
}
