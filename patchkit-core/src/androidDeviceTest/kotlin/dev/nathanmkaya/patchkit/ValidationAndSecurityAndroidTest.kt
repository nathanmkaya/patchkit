...
class ValidationAndSecurityAndroidTest : BaseAndroidPatchKitTest() {
    @Test
    fun precondition_failure_does_not_mutate_state() = runBlocking {
        ...
        val patch =
            patchJson(
                id = "expect-empty",
                preconditions =
                    listOf(conditionCount("accounts", expected = 0, label = "Table empty")),
    ...
    @Test
    fun postcondition_failure_reports_and_keeps_committed_state() = runBlocking {
        val patch =
            patchJson(
                id = "postcheck-fails",
                preconditions =
                ...
                postconditions =
                    listOf(conditionCount("accounts", expected = 2, label = "Expect two")),
            )
    ...
    @Test
    fun multi_statement_patch_is_rejected() = runBlocking {
        val patch =
            patchJson(
                id = "multi-statement",
    ...
    @Test
    fun select_action_is_rejected() = runBlocking {
        val patch = patchJson(id = "select-action", actions = listOf(sql("SELECT * FROM accounts")))
    ...
    @Test
    fun hash_mismatch_fails_validation() = runBlocking {
        val patch =
            patchJson(
                id = "hash-mismatch",
                preconditions = listOf(conditionCount("accounts", expected = 0, label = "Empty")),
    ...
    @Test
    fun precondition_non_numeric_fails_with_descriptive_message() = runBlocking {
        val patch =
            patchJson(
                id = "non-numeric",
                preconditions =
    ...
    @Test
    fun unknown_target_alias_emits_patch_failure() = runBlocking {
        val patch =
            patchJson(
                id = "unknown-target",
                target = "secondary",
