...
class HappyPathIdempotencyAndroidTest : BaseAndroidPatchKitTest() {
    @Test
    fun apply_patch_creates_account_and_is_idempotent() =
        runBlocking {
            val seedPatch =
                patchJson(
                    id = "seed-account",
                    ...
                    postconditions = listOf(conditionCount("accounts", expected = 1, label = "One account")),
                )
    ...
    @Test
    fun idempotency_record_persists_metadata_and_prevents_duplicates() =
        runBlocking {
            val patch =
                patchJson(
                    id = "metadata-record",
            ...
            val replay =
                patchJson(
                    ...
                    postconditions = listOf(conditionCount("accounts", expected = 1, label = "Still one")),
    ...
    @Test
    fun preseeded_idempotency_row_skips_execution() =
        runBlocking {
            ...
            val patch =
                patchJson(
                    ...
                    postconditions = listOf(conditionCount("accounts", expected = 1, label = "Should remain empty")),
                )
