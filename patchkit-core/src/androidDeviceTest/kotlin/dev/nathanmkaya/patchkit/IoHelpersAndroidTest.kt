...
class IoHelpersAndroidTest : BaseAndroidPatchKitTest() {
    @Test
    fun apply_directory_executes_patches_in_filename_order() =
        runBlocking {
            val dir = createTempDir("patchkit-batch")
            writePatchFile(dir.resolve("001_seed.json"), patchJson(id = "001-batch", actions = listOf(insertAccount("batch-1", 1))))
            writePatchFile(dir.resolve("002_seed.json"), patchJson(id = "002-batch", actions = listOf(insertAccount("batch-2", 2))))
    ...
    @Test
    fun apply_path_returns_validation_failure_for_empty_file() =
        runBlocking {
            val file = createTempDir("empty-root").resolve("empty.json").apply {
                writeText("   ")
            }
