...
class DdlAndConfigAndroidTest : BaseAndroidPatchKitTest() {
    @Test
    fun ddl_blocked_without_flag() = runBlocking {
        val patch =
            patchJson(
                id = "create-labels",
                preconditions = listOf(conditionTableAbsent("labels")),
                actions =
                    listOf(sql("CREATE TABLE labels(id INTEGER PRIMARY KEY, name TEXT NOT NULL)")),
    ...
    @Test
    fun ddl_allowed_with_flag_creates_table() = runBlocking {
        val patch =
            patchJson(
    ...
    @Test
    fun ddl_with_postcondition_failure_leaves_table_created() = runBlocking {
        val patch =
            patchJson(
                id = "ddl-post-fail",
                preconditions = listOf(conditionTableAbsent("labels")),
                ...
                postconditions =
                    listOf(
                        conditionTableExists("labels"),
                        conditionCount("labels", expected = 1, label = "Expect row"),
                    ),
            )
    ...
    @Test
    fun checks_in_read_transactions_option_runs_successfully() = runBlocking {
        val patch =
            patchJson(
                id = "read-tx-checks",
