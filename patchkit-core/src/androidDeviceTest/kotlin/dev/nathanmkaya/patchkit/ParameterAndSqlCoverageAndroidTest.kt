...
class ParameterAndSqlCoverageAndroidTest : BaseAndroidPatchKitTest() {
    @Test
    fun parameter_round_trip_for_all_types() = runBlocking {
        ...
        val patch =
            patchJson(
                ...
                actions =
                    listOf(
                        ParameterizedSqlAction(
                            sql =
                                "INSERT INTO blobs(text_col, int_col, real_col, blob_col) VALUES (?, ?, ?, ?)",
                            parameters =
                                listOf(
                                    SqlArg.Text("hello"),
                                    SqlArg.Int64(42),
                ...
                postconditions =
                    listOf(conditionCount("blobs", expected = 1, label = "One blob row")),
            )
    ...
    @Test
    fun with_clause_insert_is_allowed() = runBlocking {
        val patch =
            patchJson(
                id = "with-insert",
                actions =
                ...
                postconditions =
                    listOf(conditionCount("accounts", expected = 1, label = "One CTE row")),
            )
